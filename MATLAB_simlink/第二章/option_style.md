...
|                    函数                     |     功能说明     |
| :---------------------------------------: | :----------: |
|        axis([Xmin,Xmax,Ymin,Ymax])        |  x,y轴范围进行调整  |
|             xlabel('string')              |   标注x轴的名称    |
|             ylabel('string')              |   标注y轴的名称    |
|              title('string')              |    标注图形标题    |
| legend('string1','string2','string3',...) |    标注图例标注    |
|                  grid on                  |   给图形增加网格    |
|                 grid off                  |   给图形取消网格    |
...
| 函数                  | 功能说明                                      |
| ------------------- | ----------------------------------------- |
| [X,Y]=meshgrid(x,y) | 根据(x,y)二维坐标数据生成xy网格点坐标数据，其中，x、y是向量，X、Y是矩阵 |
